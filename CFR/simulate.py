...
from policies import (
    attacker_shortest_path_policy,
    attacker_ambiguous_policy,
    defender_chase_belief_policy,
)
...
def simulate_one_game(
    game: GraphGame,
    true_goal: int,
    attacker_policy_name: str = "shortest",
    beta: float = 2.0,
):
    state = game.initial_state(true_goal=true_goal)
    belief = {g: 1.0 / len(game.goals) for g in game.goals}
    ...
    while not game.is_terminal(state):
        ...
        if attacker_policy_name == "shortest":
            attacker_action = attacker_shortest_path_policy(game, state)
        elif attacker_policy_name == "ambiguous":
            attacker_action = attacker_ambiguous_policy(
                game,
                state,
            ...
            raise ValueError("attacker_policy_name must be 'shortest' or 'ambiguous'")
...
def main():
    ...
    for policy_name in ["shortest", "ambiguous"]:
        for true_goal in [2, 6]:
            final_state, payoff, trajectory = simulate_one_game(
                game,
                true_goal=true_goal,
                attacker_policy_name=policy_name,
                beta=2.0,
            )

            print("\n" + "=" * 60)
            print(f"Attacker policy: {policy_name}")
            print(f"True goal: {true_goal}")
            print("Trajectory:")
            for step in trajectory:
                print(step)
